...
const AuthForm = ({ type, setPage }) => {
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      setPage('listings');
    } catch (err) { /* error already set in context */ }
  };
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-2xl mt-10">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        {type === 'register' && (
          <>
            ...
            <input name="firstName" placeholder="First Name" required onChange={handleChange} className="w-full p-3 border rounded-lg" />
        ...
        <input name="password" type="password" placeholder="Password" required onChange={handleChange} className="w-full p-3 border rounded-lg" />

        <Button type="submit" loading={loading} primary={true} className="w-full">
      ...
      <p className="text-center mt-4 text-sm">
        {type === 'login' ? (
          <>No account? <span onClick={() => setPage('register')} className="text-blue-600 cursor-pointer font-medium">Register</span></>
        ) : (
          <>Have account? <span onClick={() => setPage('login')} className="text-blue-600 cursor-pointer font-medium">Log in</span></>
        )}
      </p>
    </div>
  );
